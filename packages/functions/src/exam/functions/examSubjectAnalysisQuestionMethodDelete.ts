...
export const examSubjectAnalysisQuestionMethodDelete = authedProcedure
  .input(
    z.object({
      examId: z.string(),
      ...
      problemId: z.string(),
      questionId: z.string(),
      methodStepId: z.string(),
    }),
  )
  .mutation(
    async ({
      ctx: { session },
      input: { examId, organizationId, questionId, problemId, methodStepId },
    }) => {
      ...
      set(
        ...
        method.filter(methodStep => methodStep.id !== methodStepId),
      );
